...
2. 문서 리드를 코드·본문 전에 정확히 한 줄로 선언한다.
   ...
   사용자 재료(수치·경험·자료·결정)가 아직 없으면 리드 앞에 `humanize` 의 `사전`(핵심 3줄·재료 표)을 먼저 한다. 리드는 형식, 핵심 3줄은 내용이다.
...
7. 점검은 `references/preflight-doc.md` 를 한 번 돌리고 한 번에 고치고 끝낸다. 세 번째 패스는 없다. 종료 때 `doc_lint --stop` 이 소프트 룰을 본다. AI 티가 남았다고 느끼면 표면이 아니라 빠진 축을 본다. `humanize` 의 `진단` 으로 3축 진단과 확인 후보 표를 낸다.
