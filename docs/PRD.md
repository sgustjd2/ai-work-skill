...
| 작성일 | 2026-09-04 |
| 진행 | **M0~M5 전체 완료**(M0~M4 2026-09-04, M5 2026-09-06). 배포 v0.3.0(2026-09-23 외부 사실 갱신). M0 골격·훅, M1 docgen(문서·덱), M2 개발 스킬, M3 LLM 운영, M4 배포(플러그인 매니페스트·.mcp.json·llms.txt·eval·CI·ui 연동), M5 사람화·archify 연동. 스킬 12 + MCP 2(docgen·litellm-ops) + 훅 2 + 외부 스킬 archify(설치만). 180 테스트 PASS(+manual 1), ruff clean, 자기 검사 하드 0(104문서). 생성 FastAPI 골격 실제 pytest 통과, config.example 검증 통과, 설계서 docx·덱 pptx 실제 렌더 + check_output PASS · **M5 사람화 완료**(2026-09-06): `humanize` 스킬(3축 진단·확인 후보·핵심 3줄) + `humanize_scan.py` + 골든 09. 스킬 12 · **FR-40 archify 연동**(2026-09-06): `install.py --with-archify`, `references/diagram-tools.md`, 한국어 예제 validate·deliver·visual-check 실측 |
