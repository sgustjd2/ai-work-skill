...
`deck-write` 에 덱 제작 순서와 수정 기준을 넣었다. 스킬 수는 그대로 12개다.

### 추가

- `skills/deck-write/references/deck-method.md`: 내용 먼저 정리(장마다 핵심 문장, 유형별 파트, 어투 통일, 원문 수치 불변), 디자인 기준 네 가지(비율·색 hex·폰트·번호 붙인 레퍼런스), 본문 두 장 파일럿 뒤 전체 확장, 프롬프트 대 템플릿 방식 선택, 수정 기준(강조가 보이나, 데이터가 한눈에 들어오나)과 AI 흔적 표, 손 수정 분담. 그라데이션·아이콘·애니메이션은 기존 규칙과 충돌해 가져오지 않았다. 출처 `docs/research/sources-2026-09-24.md`.
- `deck-write` 절차 3·6·7 이 이 문서를 가리킨다.
